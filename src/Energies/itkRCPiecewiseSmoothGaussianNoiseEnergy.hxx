...
namespace itk
{
    ...
    RCPiecewiseSmoothGaussianNoiseEnergy<TLabelImage, TDataImage, TEnergyDifference>
    ::RCPiecewiseSmoothGaussianNoiseEnergy() {
        m_Radius = 10;
        m_SphereMaskForLocalEnergy = SphereMaskImageSourceType::New();
    }
    
    template<typename TLabelImage, typename TDataImage, typename TEnergyDifference>
    void RCPiecewiseSmoothGaussianNoiseEnergy<TLabelImage, TDataImage, TEnergyDifference>
    ::PrintSelf(std::ostream & os, Indent indent) const
    {
        Superclass::PrintSelf(os, indent);
        os << indent << "Local window radius: " << m_Radius << std::endl;
    }
    ...
    template<typename TLabelImage, typename TDataImage, typename TEnergyDifference>
    inline 
    typename RCPiecewiseSmoothGaussianNoiseEnergy<TLabelImage, TDataImage, TEnergyDifference>::
    ...
    ::EvaluateEnergyDifference_(IndexType aInd, 
        ...
        EnergyDifferenceType vEnergyDiff; 
        
        
        if (vVarTo <= NumericTraits<EnergyDifferenceType>::epsilon()) {
            vEnergyDiff = 0;
        } else if (vVarFrom <= NumericTraits<EnergyDifferenceType>::epsilon()) {
            vEnergyDiff = NumericTraits<EnergyDifferenceType>::max();
        } else {
            vEnergyDiff = (aImgValue - vMeanTo) * (aImgValue - vMeanTo) / (2.0 * vVarTo) +
                    0.5 * log(2.0 * M_PI * vVarTo) -
                    ((aImgValue - vMeanFrom) * (aImgValue - vMeanFrom) /
                    (2.0 * vVarFrom) +
                    0.5 * log(2.0 * M_PI * vVarFrom));
        }
