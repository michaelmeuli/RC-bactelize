...
float calculateKLMergingCriterion(float m1, float m2, float v1, 
				  float v2, int c1, int c2) {
  float m12 = (c1 * m1 + c2 * m2)/ (c1 + c2);
  float vs1 = v1 * (c1 - 1) + c1 * m1 * m1;
  float vs2 = v2 * (c2 - 1) + c2 * m2 * m2;

  float v12 = ( 1.0f / (c1 + c2 - 1.0f)) * 
    (vs1 + vs2 - (c1 + c2) * m12 * m12);
  float kdl1 = (m1 - m12) * (m1 - m12) / (2.0f * v12) + 0.5f * 
    (v1 / v12 - 1.0f - log(v1 / v12));
  float kdl2 = (m2 - m12) * (m2 - m12) / (2.0f * v12) + 0.5f * 
    (v2 / v12 - 1.0f - log(v2 / v12));
...
__kernel void energy(
...
#if DIMENSION == 2
       __read_only image2d_t label,  // the label image
       __read_only image2d_t data,   // the data image
#elif DIMENSION == 3
       ...
       float mask_radius_x,   // the mask radius 
       float mask_radius_y,   // the offset is guaranteed to be >= radius
#if DIMENSION == 3
       float mask_radius_z,
#endif
       ...
       __global uint* merge          // merging output
       )
{	
  ...
  for (int k = 0;k < region_size.z;k++) {
	...
	int4 label_neighbor = read_imagei(label, sampler, neigh_coords);

	float data_neighbor = (read_imagef(data, sampler, neigh_coords)).x;
