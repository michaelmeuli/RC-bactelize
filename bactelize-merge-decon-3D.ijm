...
// objects will not be exluded anymore as "Ext.Manager3D_Delete();"  is commented out
...
inputDir = "/media/mmeuli/WD-HD-ext4/20160408_BCG_Pasteur-Aeras_in_THP-1/data-deconvolved/in/"
...
while (filenumber < segFilesCount) {
	...
	print("Opening file1: " + filename);
	open(filename);
	titleSeg = getTitle(); 
	selectWindow(titleSeg);
	print("titleSeg 1: " + titleSeg);
	run("Set... ", "zoom=150"); 
	setLocation(100, 220);
	...
	open(inputDir + cNr + "-" + iNr + "-bacteria.tif");
	c1Title = getTitle(); 
...
//	run("3D Manager Options", "exclude_objects_on_edges_xy exclude_objects_on_edges_z distance_between_centers=10");    
	...
	File.close(lf);
	selectWindow(titleSeg);
//	Ext.Manager3D_Delete();	           //Bug!  Somehow suddenly deletes all objects
	selectWindow(c3Title);
	...
	for (object=0; object < nb_obj; object++) {
		...
		selectWindow(titleSeg);
		Ext.Manager3D_Quantif3D(object, "Mean", meanSeg);
		print(f, cNr + "\t" + iNr + "\t" + meanSeg + "\t" + (cx*width) + "\t" + (cy*height) + "\t" + (cz*depth) + "\t" + volume + "\t" + nbVox + "\t" + elon1 + "\t" + elon2 + "\t" + 1 + "\t" + d2s(mean,3) + "\t" + meanM + "\n");
	}
